...
export default function Home() {
  ...
  return (
    <div className="bg-[#0A2A5E] font-sans selection:bg-[#1FC8C8] selection:text-[#0A2A5E]">
      ...
      {/* 🧭 UPDATED NAVIGATION & UNIFIED SEARCH */}
      <nav className="fixed top-0 w-full z-[100] bg-[#0A2A5E] border-b border-white/10 px-6 py-5">
        <div className="max-w-[1600px] mx-auto flex justify-between items-center text-white relative">
          ...
          <div className="flex items-center gap-3 cursor-pointer shrink-0" onClick={() => navigateTo('home')}>
            <img src="/logo-jpg.jpg" alt="Precede Concepts Logo" className="w-10 h-10 md:w-12 md:h-12 object-contain rounded-lg bg-white/5" />
            <span className="text-lg font-black uppercase italic tracking-tighter hidden sm:block">PRECEDE CONCEPTS</span>
          </div>

          {/* NEW: Unified Search Input inside Navbar (Desktop) */}
          <div className="hidden lg:block flex-1 max-w-sm mx-8 relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#1FC8C8]" size={16} />
            <input
              type="text"
              ...
              onChange={(e) => {
                ...
                setSearchQuery(e.target.value); // Syncs with the hub filter dynamically
              }}
              onKeyDown={handleGlobalSearchEnter}
              className="w-full bg-[#0A2A5E]/60 border border-white/10 rounded-full py-2.5 pl-12 pr-6 text-white text-[10px] font-black uppercase italic tracking-[0.1em] outline-none focus:border-[#1FC8C8] transition-all shadow-inner"
            />
          </div>

          <div className="hidden xl:flex items-center gap-6 text-[10px] font-black uppercase tracking-[0.15em] shrink-0">
          ...
          <button className="xl:hidden ml-auto" onClick={() => setIsMenuOpen(!isMenuOpen)}><Menu size={32} /></button>
        </div>
      </nav>

      {/* NEW: Updated Mobile Menu to include search */}
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed inset-0 z-[200] bg-[#0A2A5E] flex flex-col p-8 text-white uppercase italic font-black">
            ...
            <div className="relative w-full mb-10">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  handleGlobalSearchEnter(e);
                  if (e.key === 'Enter') setIsMenuOpen(false);
                }}
                className="w-full bg-white/5 border border-white/10 rounded-full py-4 pl-14 pr-6 text-white text-[12px] font-black uppercase italic tracking-[0.1em] outline-none focus:border-[#1FC8C8] transition-all"
              />
            </div>

            <div className="flex flex-col gap-8 text-2xl">
              <button onClick={() => navigateTo('home')} className="text-left">HOME</button>
              <button onClick={() => navigateTo('about')} className="text-left">ABOUT</button>
              ...
              <button onClick={() => { setView('marketplace'); setIsMenuOpen(false); window.scrollTo(0, 0); }} className="text-left">MARKETPLACE</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence mode="wait">
        {view === 'home' ? (
          <motion.div key="home-view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <section id="services" className="min-h-screen bg-white flex flex-col justify-center py-32 px-6">
            ...
            <section id="hub" className="min-h-screen py-32 px-6 bg-[#0F4C81]">
              <div className="max-w-[1500px] mx-auto text-left text-white">
                ...
                {/* UPDATED: Cleaned up Hub controls since Search is now in Navbar */}
                <div className="flex flex-col gap-6 mb-16 bg-white/5 border-4 border-white/10 p-6 md:p-8 rounded-[2rem] shadow-2xl">
