...
export default function Home() {
  ...
  return (
    <div className="bg-[#0A2A5E] font-sans text-slate-950 scroll-smooth overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-[100] bg-[#0A2A5E]/90 backdrop-blur-md border-b border-white/10 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center text-white">
          ...
          <div className="hidden xl:flex items-center gap-5 text-[9px] font-black uppercase tracking-[0.2em] text-white/70">
            ...
            <button onClick={() => handleNavFilter('place')}>Places & Spaces</button>
      ...
      {/* --- OPPORTUNITY HUB (P N OP.PNG) --- */}
      <section id="hub" className="min-h-screen py-32 px-6 bg-[#0F4C81]">
        <div className="max-w-[1400px] mx-auto text-left">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <h2 className="text-4xl font-black uppercase italic text-white tracking-tighter italic">Opportunity Hub</h2>
            <div className="flex flex-wrap justify-center gap-2 bg-black/20 p-1.5 rounded-full">
              {['all', 'training', 'job', 'event', 'place'].map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`px-5 py-2 rounded-full text-[8px] font-black uppercase tracking-widest ${filter === f ? 'bg-[#1FC8C8] text-[#0A2A5E]' : 'text-white/60'}`}>{f === 'place' ? 'Places & Spaces' : f}</button>
              ))}
            </div>
          </div>
      ...
      {/* --- ABOUT US (P3.PNG STYLE) --- */}
      <section id="about" className="min-h-screen bg-[#1FC8C8] flex items-center justify-center px-6">
        <div className="max-w-7xl w-full flex flex-col lg:flex-row gap-16 items-center text-[#0A2A5E] text-left">
          <div className="lg:w-1/3">
            <h2 className="text-6xl md:text-8xl font-black uppercase italic tracking-tighter leading-[0.85]">
              BEYOND A <br /> DIGITAL <br /> AGENCY.
            </h2>
          </div>
          <div className="lg:w-2/3 border-l-8 border-[#0A2A5E] pl-8 flex flex-col gap-8">
            <p className="font-black text-2xl md:text-3xl italic leading-tight uppercase">
              Precede Concepts bridges the gap between high-end professional services and accessible solutions in Ghana. 
              We operate a dual-purpose ecosystem: a primary business executing top-tier digital solutions and business support services, 
              and a CSR hub curating vital community resources.
            </p>
            <div className="flex flex-col gap-4 text-left">
              <h3 className="font-black text-lg uppercase tracking-widest text-[#0A2A5E]/80">Empowering the Ghanaian Hustle</h3>
              <p className="text-lg font-bold leading-relaxed opacity-90">
                Based in Accra, we empower individuals and businesses by providing the digital, creative, and business operations support 
                needed to scale. We provide smart solutions that simplify your operations and amplify your brand. At Precede, we don’t 
                just offer services—we build the systems that help you lead.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
               <div className="flex items-center gap-3"><CheckCircle2 className="text-[#0A2A5E]"/><span className="font-black uppercase text-sm italic">Business Growth & Registration</span></div>
               <div className="flex items-center gap-3"><CheckCircle2 className="text-[#0A2A5E]"/><span className="font-black uppercase text-sm italic">Identity & Branding</span></div>
               <div className="flex items-center gap-3"><CheckCircle2 className="text-[#0A2A5E]"/><span className="font-black uppercase text-sm italic">Tech & AI Innovation</span></div>
               <div className="flex items-center gap-3"><CheckCircle2 className="text-[#0A2A5E]"/><span className="font-black uppercase text-sm italic">Computing Capacity Building</span></div>
            </div>
          </div>
        </div>
      </section>

      {/* --- SERVICES (P SERVICES.PNG) --- */}
      <section id="services" className="min-h-screen py-32 bg-white px-6">
        <div className="max-w-7xl mx-auto text-left">
          <div className="mb-16"><h2 className="text-5xl font-black uppercase italic text-[#0A2A5E]">Our Services.</h2><p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.5em]">Comprehensive Business Solutions</p></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
            {services.map((s, i) => (
              <div key={i} className="p-10 bg-slate-50 border-2 border-slate-100 rounded-[3rem] transition-all flex flex-col text-left group hover:shadow-2xl">
      ...
      <section id="contact" className="min-h-screen bg-[#0A2A5E] flex items-center justify-center px-6 text-white text-left">
        <div className="max-w-7xl w-full grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="bg-white/5 p-10 rounded-[4rem] border-4 border-white/10 shadow-2xl backdrop-blur-md">
            <div className="space-y-8 mb-10">
              <div className="flex items-center gap-6">
                <div className="p-5 bg-[#1FC8C8]/20 rounded-2xl text-[#1FC8C8] shadow-inner"><Phone size={32}/></div>
                <div><span className="text-[10px] font-black uppercase text-white/40">Call Us</span><p className="text-4xl font-black italic tracking-tighter">{BUSINESS_PHONE}</p></div>
              </div>
              <div className="flex items-center gap-6">
                <div className="p-5 bg-[#1FC8C8]/20 rounded-2xl text-[#1FC8C8] shadow-inner"><Mail size={32}/></div>
                <div><span className="text-[10px] font-black uppercase text-white/40">Email Us</span><p className="text-2xl font-black italic tracking-tighter">{BUSINESS_EMAIL}</p></div>
              </div>
            </div>
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <a href={`https://wa.me/233591999544`} target="_blank" className="bg-white text-[#0A2A5E] p-6 rounded-[2rem] font-black uppercase italic text-sm flex items-center justify-center gap-3 hover:bg-[#1FC8C8] transition-all shadow-xl"><WhatsAppIcon /> Whatsapp</a>
                <a href={`https://whatsapp.com/channel/0029Vb7Mfjf5EjxpZuIIpA2W`} target="_blank" className="bg-white/10 border-4 border-white/10 p-6 rounded-[2rem] font-black uppercase italic text-sm flex items-center justify-center gap-3 hover:bg-white transition-all"><Smartphone size={20}/> Join Channel</a>
              </div>
              <a href={`mailto:${BUSINESS_EMAIL}`} className="bg-[#1FC8C8] text-[#0A2A5E] p-7 rounded-[2.5rem] font-black uppercase italic text-lg flex items-center justify-center gap-4 shadow-2xl hover:bg-white transition-all">
      ...
      {/* --- BACK TO TOP --- */}
      <AnimatePresence>{showBackToTop && <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="fixed bottom-10 right-10 z-[150] p-6 bg-[#1FC8C8] text-[#0A2A5E] rounded-3xl shadow-2xl hover:scale-110 transition-all"><ArrowUp size={28} /></motion.button>}</AnimatePresence>
    </div>
  )
}
  ...
  return (
    <div className="group bg-white rounded-[2rem] overflow-hidden flex flex-col shadow-lg border border-slate-100 h-full transition-all hover:scale-[1.03]">
      <div className="h-24 bg-slate-900 relative">
        ...
        <span className="absolute top-2 left-2 text-[7px] font-black bg-[#1FC8C8] text-[#0A2A5E] px-2 py-0.5 rounded-full uppercase italic tracking-widest">{item.category}</span>
      </div>
